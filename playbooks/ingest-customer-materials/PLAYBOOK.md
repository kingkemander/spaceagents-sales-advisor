...
11. 在最终确认卡中主动展示客户资料完整度、缺失项、影响和最自然的补问方式。即使用户选择“先录入”，缺失项也不得消失。
12. 如果用户选择先录入且资料严重缺失，继续读取 `<运行时根目录>/playbooks/schedule-sales-reminders/PLAYBOOK.md`，为销售本人创建“补全客户资料｜客户名”任务。默认提醒时间为下一工作日上午 10:00；若已有明确下次联系时间，则在该时间前 30 分钟提醒。任务中列出最重要的 3 项缺口和自然补问，不要求一次问完。
...
检查来源渠道、姓名/联系方式、公司全称、客户职位、所属行业、核心需求、预算、项目周期、决策关系和下一次跟进时间：

- **轻度缺失**：缺少 1–2 个非关键字段。确认卡提醒，正常录入。
- **中度缺失**：缺少 3–4 个字段，但客户身份与核心需求已知。确认卡给出补充顺序，正常录入。
- **严重缺失**：完整度低于 50%，或“公司/身份”与“核心需求”同时缺失，或没有可识别联系人且决策关系未知。允许用户选择先录入，但必须创建补全任务。

补全任务只提醒销售，不自动联系客户。若当前环境没有自动任务能力，把任务写入 `indexes/reminder-index.json` 并明确说明尚未建立系统提醒。
