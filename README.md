...
在 GitHub Releases 下载最新的 `spaceagents-sales-advisor-v0.14.0.zip`，然后直接上传到 Space Agents 的技能/插件导入页面。上传包只包含运行必需文件，不携带宣传图或案例大图，避免安装材料进入模型上下文。
...
- 首次对话会以每次一个问题的方式主动了解销售业务；已回答或跳过的信息不会反复追问，明确任务永远优先。
- 支持“随手记”：销售随口汇报客户变化后，军师先给确认卡，确认后再更新客户记忆、今日行动与资料补全建议。
- 支持待确认建议队列、军师自校准、周经营复盘、行业适配配置和合规候选线索发现；候选线索与正式客户档案隔离，人工确认后才转化。
...
- 支持“客户企业动态雷达”：按法定全称、统一社会信用代码和地区核验企业，覆盖招投标、中标、备案环评、土地许可、工商信用、融资招聘、扩产搬迁、司法风险、知识产权和官方新闻。
- 外部动态分为已核验、待核验和主体冲突三层；搜索摘要不能进入客户事实，统一社会信用代码冲突会被代码直接拒绝。
...
- macOS 将固定播放器部署到 `~/Library/Application Support/SalesVoiceAlarm/`，由 `launchd + say` 到点实时播报，不再读取 Downloads 工作区里的脚本或音频；Windows 使用系统语音生成 WAV，再由任务计划程序 + `SoundPlayer` 到点播放。默认只有语音，没有桌面横幅、通知栏气泡或任务栏弹窗；一次性提醒播完自动清理。
...
当前版本：`v0.14.0`。新增主动式销售军师：渐进了解销售业务、随手记确认更新、待确认建议队列、自校准、周经营复盘、行业适配与合规候选线索发现；继续保留企业动态雷达、个人口吻、专业回复、macOS/Windows 语音提醒、自动更新、标准销售漏斗、可显示销售军师智能体与用户数据保护。
