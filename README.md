...
在 GitHub Releases 下载最新的 `spaceagents-sales-advisor-v0.13.1.zip`，然后直接上传到 Space Agents 的技能/插件导入页面。上传包只包含运行必需文件，不携带宣传图或案例大图，避免安装材料进入模型上下文。
...
当前版本：`v0.13.1`。修复 macOS Downloads 权限导致的语音提醒失效：固定播放器部署到 Application Support，由 `launchd + say` 到点实时播报，不再让后台进程读取工作区脚本或音频。继续保留企业动态雷达、个人口吻、专业回复、Windows 语音提醒、自动更新、标准销售漏斗、可显示销售军师智能体与用户数据保护。
