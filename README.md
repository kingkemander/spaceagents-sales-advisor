...
在 GitHub Releases 下载最新的 `spaceagents-sales-advisor-v0.13.0.zip`，然后直接上传到 Space Agents 的技能/插件导入页面。上传包只包含运行必需文件，不携带宣传图或案例大图，避免安装材料进入模型上下文。
...
当前版本：`v0.13.0`。新增客户企业动态雷达：按法定主体查询并核验招投标、项目、经营、资本、招聘、司法、知识产权与官方新闻；100 条以内候选全部核验，已核验动态可进入作战台，销售确认后才写入正式客户时间线。继续保留个人口吻、专业回复、语音提醒、自动更新、标准销售漏斗、可显示销售军师智能体与用户数据保护。
