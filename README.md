...
在 GitHub Releases 下载最新的 `spaceagents-sales-advisor-v0.8.0.zip`，然后直接上传到 Space Agents 的技能/插件导入页面。
...
- 该入口根据用户意图自动读取七个内部 Playbook，不需要用户选择子技能。
...
当前版本：`v0.8.0`。新增文字优先的销售行动提醒、到点完成确认、每日自动简报和未来半年排期；客户看板改为按需生成，不再作为提醒的默认入口。
