...
```text
<工作区>/.spaceagents/plugins/sa-sales-advisor/runtime-v0.6.2/
```

每次触发时，先检查以下文件是否存在：

```text
<工作区>/.spaceagents/plugins/sa-sales-advisor/runtime-v0.6.2/sa_sales_advisor/cli.py
```
...
```bash
python3 -c "import hashlib,pathlib,urllib.request; u='https://raw.githubusercontent.com/kingkemander/spaceagents-sales-advisor/v0.6.2/bootstrap.py'; p=pathlib.Path(r'<工作区>')/'.spaceagents/plugins/sa-sales-advisor/bootstrap-v0.6.2.py'; p.parent.mkdir(parents=True,exist_ok=True); d=urllib.request.urlopen(u,timeout=60).read(); h=hashlib.sha256(d).hexdigest(); assert h=='3a5439164cefe34faeb1a229c5748cbbc2d74835d6316af74fb0dc237d8b4cb8', f'bootstrap checksum mismatch: {h}'; p.write_bytes(d); print(p)"
```

第二步，运行引导器。它会下载并校验运行时包，然后返回真实 CLI 路径：

```bash
python3 "<工作区>/.spaceagents/plugins/sa-sales-advisor/bootstrap-v0.6.2.py" --workspace "<工作区>"
```
...
```text
<运行时根目录> = <工作区>/.spaceagents/plugins/sa-sales-advisor/runtime-v0.6.2
