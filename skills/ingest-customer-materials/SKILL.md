...
```bash
python3 scripts/init_workspace.py --root "<当前项目>/SA销售工作区"
```
...
7. 用户选择新客户时，运行 `scripts/customer_store.py create` 创建稳定 ID 和目录。
8. 用户确认后，复制原件到 `sources/original/`，保存 Markdown 到 `sources/markdown/`，创建时间线记录，再运行 `register-material` 登记索引和流水。
9. 调用 `$maintain-customer-memory`，根据本次已确认事实更新客户状态。
10. 运行 `scripts/customer_store.py validate`，确认 customer_id、路径和索引一致。
